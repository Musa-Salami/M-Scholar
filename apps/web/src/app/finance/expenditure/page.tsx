...
import { EXPENSE_CATEGORY_LABELS, FINANCE_NAV, type ExpenseCategory } from "@m-scholar/shared";
...
export default function ExpenditurePage() {
  ...
  const { expenditure, addExpenditure } = useFinanceStore();
  ...
  const total = expenditure.reduce((s, r) => s + r.amount, 0);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setForm({ category: "other", description: "", amount: "", date: new Date().toISOString().slice(0, 10), reference: "" });
    setShowForm(false);
  };
  ...
  return (
    <PortalShell navItems={FINANCE_NAV} title="Finance Officer Portal">
      ...
      <div className="card-shadow mb-6 rounded-2xl border border-red-100 bg-red-50 p-5">
        <p className="text-sm text-red-700">Total expenditure recorded</p>
        <p className="font-display text-3xl font-bold text-red-800">{formatCurrency(total)}</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="card-shadow mb-6 rounded-2xl border border-slate-100 bg-white p-6">
          <div className="grid gap-4 sm:grid-cols-2">
            <FormField label="Category">
              <select className={selectClass} value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value as ExpenseCategory })}>
      ...
      <DataTable headers={["Date", "Category", "Description", "Reference", "Amount"]}>
        {[...expenditure].reverse().map((r) => (
          <tr key={r.id} className="hover:bg-slate-50">
            <td className="px-6 py-4 text-slate-500">{r.date}</td>
            <td className="px-6 py-4">{EXPENSE_CATEGORY_LABELS[r.category]}</td>
            <td className="px-6 py-4">{r.description}</td>
            <td className="px-6 py-4 text-slate-500">{r.reference}</td>
            <td className="px-6 py-4 font-medium text-red-700">{formatCurrency(r.amount)}</td>
          </tr>
        ))}
      </DataTable>
    </PortalShell>
